...
import { GenericMoreButton } from '@components/atomic';
...
function UserManagementTable(props) {
  ...
  const handleSelectAll = (event) => {
    const selectedCust = event.target.checked
      ? customers.map((customer) => customer.id)
      : [];

    setSelectedCustomers(selectedCust);
  };
  ...
  return (
    <div {...rest} className={classes.root}>
      ...
      <TableEditDrawer selected={selectedCustomers} />
    </div>
  );
}
